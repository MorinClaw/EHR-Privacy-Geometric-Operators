...
# -*- coding: utf-8 -*-
"""
Weak-interp A2 runner (z-only, n_passes=5 only).

Purpose:
  For alpha=2/3, only generate setting=z perturbed outputs, and only keep T1 n_passes=5.
  This is much faster than the full A2 grid (z+phys, n_passes=2/5/10).

Input:
  ts_dir: P3_weak_interpolation/ts_48h_weak (must contain ts_single_column_{var}_zscore.csv)
Output:
...
def load_single_column_z(ts_dir: Path, var: str) -> np.ndarray:
    ...
    if not path.exists():
        # allow ts_48h_weak but file name without prefix
        path = ts_dir / f"ts_single_column_{var}.csv"
    if not path.exists():
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Run weak A2 z-only (n_passes=5) on ts_48h_weak")
    ...
    for var in args.variables:
        ...
        if "T1_uniform" in args.operators:
            ...
            if (not skip) or (not out_path.exists()):
                ...
                print(f"[weak z-only] {var} T1_uniform -> {out_path.name}")
        ...
        if "T1_weighted" in args.operators:
            ...
            if (not skip) or (not out_path.exists()):
                ...
                pd.DataFrame({"value": to_output(y)}).to_csv(out_path, index=False)
                print(f"[weak z-only] {var} T1_weighted -> {out_path.name}")
        ...
        if "T2" in args.operators:
            ...
            if (not skip) or (not out_path.exists()):
                ...
                pd.DataFrame({"value": to_output(y)}).to_csv(out_path, index=False)
                print(f"[weak z-only] {var} T2 -> {out_path.name}")
